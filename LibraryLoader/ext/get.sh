...
fn_get()
{
    ...
    cd "${BASE_DIR}/src" && curl -L "${URL}" -o "${NAME}" --progress-bar
    cd "${BASE_DIR}"
}
...
fn_get_tgz()
{
    ...
    BASE_DIR=`pwd`
    fn_get tmp.tgz "${URL}"
    cd "${BASE_DIR}/src" && tar -xzf tmp.tgz && rm -f tmp.tgz
    cd "${BASE_DIR}"
}
...
fn_get_tgz https://www-eu.apache.org/dist/apr/apr-1.6.5.tar.gz
